...
Doorkeeper.configure do
	resource_owner_from_credentials do |routes|
	  Rails.logger.info "[Doorkeeper] Resource Owner Auth: #{params[:username]} / #{params[:password]}"
	  
	  user = User.find_by(email: params[:username])
	  
	  if user
	    Rails.logger.info "[Doorkeeper] Found User: #{user.email}"
	    if user.valid_password?(params[:password])
	      Rails.logger.info "[Doorkeeper] Password OK for #{user.email}"
	      user
	    else
	      Rails.logger.warn "[Doorkeeper] Invalid password for #{user.email}"
	      nil
	    end
	  else
	    Rails.logger.warn "[Doorkeeper] No user found for #{params[:username]}"
	    nil
	  end
	end
